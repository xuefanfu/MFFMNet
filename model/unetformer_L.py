...
from .select_feature_L import BiAttentionBlock
from .swin_transformer_L import SwinTransformer
from .swin_transformer_unet_skip_expand_decoder_sys import SwinTransformerSysfinal_decoder, SwinTransformerSys
import copy
...
#         self.select_feature_layer = BiAttentionBlock(img_dim=256, d_dim=256, embed_dim=256, num_heads=1)
#         self.select_layers = _get_clones(self.select_feature_layer, select_num)

#         self.conv_downsample = Conv(256,256)
#         self.conv_downsample_layers = _get_clones(self.conv_downsample, select_num-1)

#         self.features_proj = nn.Linear(img_dim*2, img_dim)
#         self.features_proj_layers = _get_clones(self.features_proj, select_num-1)
...
#         x_local_features_up_sampled = [upsample(f, (64, 64)) for f in x_local_features_same_dim]
#         x_local_features_merged = torch.cat(x_local_features_up_sampled, dim=1)
#         # x_local_features_merged_dd = x_local_features_merged
#         # ppm_out = []
        
#         # for p in self.pool_layers:
#         #     pool_out = p(x_local_features_merged)
#         #     pool_out = F.interpolate(pool_out, (64, 64), mode='bilinear', align_corners=False)
#         #     ppm_out.append(pool_out)
#         # x_local_features_merged = torch.cat((x_local_features_merged_dd, torch.cat(ppm_out, dim=1)), dim=1)
#         # x_local_features_merged = self.shift_dim(x_local_features_merged)

#         # print("x_local_features_up_sampled", x_local_features_merged.shape)
...
#         # print("x_local_features_merged", x_local_features_merged.shape)
...
#             d_select, atten_img2d = select(img_querys[i], y_local_features_embed[i])
#             select_d_features.append(d_select)
#             select_attens.append(atten_img2d)
        
#         new_select_d_features = []
#         for i, select_d_f in enumerate(select_d_features):
#             if self.d_select_proj_layers[i] is not None:
#                 select_d_f = self.d_select_proj_layers[i](select_d_f)
#             new_select_d_features.append(select_d_f)

#         # new_select_d_features = None
#         encode_result = self.encoder(x_local_features_merged, x_new, new_select_d_features)        
...
class UNetFormer(nn.Module):
    def __init__(self,
        ...
        self.select_feature_layer = BiAttentionBlock(img_dim=256, d_dim=256, embed_dim=1024, num_heads=4)
        self.select_layers = _get_clones(self.select_feature_layer, select_num)
        ...
        self.feature_merged_proj = nn.Linear(2048, 96)
    ...
    def forward(self, x, y):
        ...
        for i, select in enumerate(self.select_layers):
            ...
            if s_d**0.5 != 64:
                d_select = upsample(d_select, (64, 64))
            select_d_features.append(d_select)
            select_attens.append(atten_img2d)
        
        d_select = torch.cat(select_d_features, dim=1)
        high_feature = torch.cat((x_local_features_merged, d_select), dim=1)
        high_feature = self.feature_merged_proj(high_feature.flatten(2).permute(0,2,1))
        
        new_select_d_features = None
        encode_result = self.encoder(high_feature, x_new, new_select_d_features)        
...
#         self.select_feature_layer = BiAttentionBlock(img_dim=img_dim, d_dim=img_dim, embed_dim=768, num_heads=4)
#         self.select_layers = _get_clones(self.select_feature_layer, select_num)

#         self.conv_downsample = Conv(64, 64)
#         self.conv_downsample_layers = _get_clones(self.conv_downsample, select_num-1)
...
#         # self._proj = nn.Linear(3840, 96)
#         self.feature_merged_proj = nn.Linear(256, 96)
...
#                 features_embed = self.encode_sw_proj_layers[i](features_embed)
#             x_new.append(features_embed)
#             # d_select torch.Size([10, 4096, 64])
#             # d_select torch.Size([10, 1024, 64])
#             # d_select torch.Size([10, 256, 64])
#             # d_select torch.Size([10, 64, 64])
#         select_d_features = []
#         select_attens = []
#         for i, select in enumerate(self.select_layers):
#             if i == 0:
#                 d_select, atten_img2d = select(img_querys[i], y_local_features_embed[i])
#             else:
#                 d_select = avg_pool1d(d_select.permute(0, 2, 1), kernel_size=4, stride=4).permute(0, 2, 1)
#                 query = torch.cat((img_querys[i], d_select), dim=1)
#                 query = avg_pool1d(query.permute(0, 2, 1), kernel_size=2, stride=2).permute(0, 2, 1)
#                 d_select, atten_img2d = select(query, y_local_features_embed[i])
#             select_d_features.append(d_select)
#             select_attens.append(atten_img2d)
