...
from .select_feature import BiAttentionBlock
...
class UNetFormer(nn.Module):
    def __init__(self,
        ...
        # # for i in range(len(backbone_channels)-1):
        # #     if i != 0:

        # # 创建上采样层实例
        # self.upsample_layers = nn.ModuleList([
        #     None,  # 第一个张量已经是 64x64，不需要上采样
        #     create_upsample_layer(512, 256, 2),  # 上采样至 64x64
        #     create_upsample_layer(1024, 256, 4), # 上采样至 64x64
        #     create_upsample_layer(2048, 256, 8)  # 上采样至 64x64
        # ])

        encoder_channels = self.backbone.feature_info.channels()
        # self.decoder = SwinTransformerSys(num_classes=6)
        self.final_decoder = SwinTransformerSysfinal_decoder(num_classes=6)
        # self.decoder = Decoder(encoder_channels, decode_channels, dropout, window_size, num_classes)
    ...
    def forward(self, x, y):
        ...
        # x_new = []
        # # 依次处理每个张量
        # for i, mid_feature in enumerate(res):
        #     if self.upsample_layers[i] is not None:
        #         mid_feature = self.upsample_layers[i](mid_feature)
        #     x_new.append(mid_feature)

        # # 在通道维度上拼接张量
        # x_new = torch.cat(x_new, dim=1)
        # x_new = self._proj(x_new.flatten(2).permute(0,2,1))
        ...
        #         y = y.flatten(2).permute(0, 2, 1)
        #         y_downsample_d.append(y) 
        #     result, resultd = self.decoder(x, x_downsample,y, y_downsample_d)d
