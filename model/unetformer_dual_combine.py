...
from .swin_transformer_unet_skip_expand_decoder_sys_dual import SwinTransformerSysfinal_decoder, SwinTransformerSys
import copy
...
class FeaturePyramid(nn.Module):
    def __init__(self, in_channels_list, out_channels):
        ...
        # 对每个尺度的特征图进行1x1卷积，调整通道数
        self.lateral_convs = nn.ModuleList([
            nn.Conv2d(in_channels, out_channels, kernel_size=1)
            for in_channels in in_channels_list
        ])
        # 上采样后的卷积，用于进一步处理上采样的特征图
    ...
    def forward(self, features):
        # 获取不同尺度的特征图
        p3, p4, p5 = features  # 假设p3是32x32, p4是16x16, p5是8x8
        
        # 对每个尺度的特征图进行1x1卷积
        p5_1x1 = self.lateral_convs[2](p5)
        p4_1x1 = self.lateral_convs[1](p4)
        p3_1x1 = self.lateral_convs[0](p3)
        
        # 自顶向下的路径
        # 先对p5进行上采样，然后与p4进行逐元素相加
        p4_upsampled = F.interpolate(p5_1x1, scale_factor=2, mode='nearest')
        p4_combined = p4_1x1 + p4_upsampled
        
        # 再对p4进行上采样，然后与p3进行逐元素相加
        p3_upsampled = F.interpolate(p4_combined, scale_factor=2, mode='nearest')
        p3_combined = p3_1x1 + p3_upsampled
        
        # 对组合后的特征图进行3x3卷积，平滑特征图
        p5_smooth = self.smooth_convs[2](p5_1x1)
        p4_smooth = self.smooth_convs[1](p4_combined)
        p3_smooth = self.smooth_convs[0](p3_combined)
        
        # 返回最底层的特征图，即32x32的特征图
        return p3_smooth
...
class UNetFormer(nn.Module):
    def __init__(self,
        ...
        self.select_all_feature_proj = ConvBN(512,256)

        self.project_basefeature_d = nn.Conv2d(img_dim, img_dim, kernel_size=1, bias=bias)
        self.project_detailfeature_d = nn.Conv2d(img_dim, img_dim, kernel_size=1, bias=bias)
        self.project_basefeature_d_condv = nn.Conv2d(img_dim, img_dim, kernel_size=3, stride=1, padding=1, bias=bias)
        self.project_detailfeature_d_condv = nn.Conv2d(img_dim, img_dim, kernel_size=3, stride=1, padding=1, bias=bias)
    
        self.gate = nn.Conv2d(img_dim*2, img_dim, kernel_size=3, stride=1, padding=1, bias=bias)
        self.fre_feature_project_forward = nn.Sequential(*[nn.Conv2d(img_dim, img_dim, kernel_size=3, stride=1, padding=1,  bias=bias),
                                    nn.Conv2d(img_dim, img_dim, kernel_size=3, stride=1, padding=1,  bias=bias)])
        self.fre_feature_project_back = nn.Sequential(*[nn.Conv2d(img_dim, img_dim, kernel_size=3, stride=1, padding=1,  bias=bias),
                                    nn.Conv2d(img_dim, img_dim, kernel_size=3, stride=1, padding=1,  bias=bias)])
        self.spatial_atten = SpatialAttention(kernel_size=7)
        ...
        # 创建模型实例
        self.feature_pyramid = FeaturePyramid(in_channels_list, out_channels)
    ...
    def forward(self, x, y):
        ...
        d_select = self.feature_pyramid(select_d_features)
        ...
        slect_basefeature_d = self.project_basefeature_d(slect_basefeature_d)
        slect_basefeature_d = self.project_basefeature_d_condv(slect_basefeature_d)
        slect_detailfeature_d = self.project_detailfeature_d(slect_detailfeature_d)
        slect_detailfeature_d = self.project_detailfeature_d_condv(slect_detailfeature_d)
        select_fre_feature = torch.cat((slect_basefeature_d, slect_detailfeature_d),dim=1)
        gate = self.gate(select_fre_feature)
        select_fre_feature = gate * slect_basefeature_d + (1 - gate) * slect_detailfeature_d
        select_fre_feature = self.fre_feature_project_forward(select_fre_feature)
        select_fre_feature_atten = self.spatial_atten(select_fre_feature)
        select_fre_feature = select_fre_feature * select_fre_feature_atten
        select_fre_feature = self.fre_feature_project_back(select_fre_feature)
        
        slect_all_feature = [select_fre_feature, d_select]
